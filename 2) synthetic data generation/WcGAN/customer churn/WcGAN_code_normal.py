...
cluster_spec = importlib.util.spec_from_file_location("k means classes_normal.py",
                                                      "C:/Users/amarek/PycharmProjects/MSc Project/"
                                                      "2) synthetic data generation/cGAN/customer churn/"
                                                      "k means classes_normal.py")

cluster = importlib.util.module_from_spec(cluster_spec)
cluster_spec.loader.exec_module(cluster)

normal_w_classes = cluster.fraud_w_classes
data = normal_w_classes
...
def WcGAN_generate_data(data=data, rand_dim=42, base_n_count=128, nb_steps=6000 + 1, batch_size=256, k_d=5, k_g=1,
                      critic_pre_train_steps=100, log_interval=100, learning_rate=1e-3,
                      data_dir='2) synthetic data generation/WcGAN/customer churn/WcGAN training/normal_',
                      gen_data_size=len(data), gen_data_name='WcGAN_normal_5174'):
    ...
    label_cols = [i for i in normal_w_classes.columns if 'class' in i]
    data_cols = [i for i in normal_w_classes.columns if i not in label_cols]

    # adversarial_training_WGAN(arguments, normal_w_classes, data_cols=data_cols, label_cols=label_cols)  # CGAN
    ...
    df.to_pickle('2) synthetic data generation/WcGAN/customer churn/WcGAN results/'+gen_data_name+'.pkl')
    ...
    plt.legend()
    plt.title('WcGAN training')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    # plt.xticks(np.arange(0,nb_steps, step=log_interval))
    plt.savefig('2) synthetic data generation/WcGAN/customer churn/WcGAN results/'+gen_data_name+'.png',
                bbox_inches='tight')
    # plt.show()

    with open('2) synthetic data generation/WcGAN/customer churn/WcGAN results/'+gen_data_name+'.txt','w')as a:
