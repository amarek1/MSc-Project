...
def WGAN_generate_data(data=data, rand_dim=32, base_n_count=128, nb_steps=6000 + 1, batch_size=128, k_d=5, k_g=1,
                      ...
                      data_dir='2) synthetic data generation/WGAN/bioresponse/WGAN training/normal_',
                      gen_data_size=3000, gen_data_name='WGAN_bio_0_3000'):
    ...
    df.to_pickle('2) synthetic data generation/WGAN/bioresponse/'+gen_data_name+'.pkl')
    ...
    plt.legend()
    plt.title('WGAN training - bioresponse dataset')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    # plt.xticks(np.arange(0,nb_steps, step=log_interval))
    plt.savefig('2) synthetic data generation/WGAN/bioresponse/'+gen_data_name+'.png')
    plt.show()

    with open('2) synthetic data generation/WGAN/bioresponse/'+gen_data_name+'.txt','w')as a:
