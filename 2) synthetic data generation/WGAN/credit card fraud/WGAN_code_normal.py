...
normal_data = ori_data.loc[ori_data['class'] == 0]
data = normal_data
...
def WGAN_generate_data(data=data, rand_dim=32, base_n_count=128, nb_steps=6000 + 1, batch_size=128, k_d=5, k_g=1,
                      ...
                      data_dir='2) synthetic data generation/WGAN/credit card fraud/WGAN training/normal_',
                      gen_data_size=len(data), gen_data_name='WGAN_normal_284315'):
    ...
    df.to_pickle('2) synthetic data generation/WGAN/credit card fraud/'+gen_data_name+'.pkl')
    ...
    plt.legend()
    plt.title('WGAN training - Adam optimizer')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    # plt.xticks(np.arange(0,nb_steps, step=log_interval))
    plt.savefig('2) synthetic data generation/WGAN/credit card fraud/'+gen_data_name+'.png')
    plt.show()

    with open('2) synthetic data generation/WGAN/credit card fraud/'+gen_data_name+'.txt','w')as a:
