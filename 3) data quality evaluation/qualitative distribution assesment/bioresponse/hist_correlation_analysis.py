...
def compare_data(real_data, synthetic_data, datatype, generator):
    ...
    #            transform=axes[i].transAxes)
    #
    # f.savefig('3) data quality evaluation/qualitative distribution assesment/bioresponse/histograms/histograms_' + generator + '.png', bbox_inches='tight')
    ...
    correlation_matrix(real_corr - synthetic_corr, 'Correlation Difference for ' + generator+'\n Absolute mean correlation difference: '+ str(abs_mean_corr),
                       '3) data quality evaluation/qualitative distribution assesment/bioresponse/correlation plots/correlation_' + generator)
    ...
    summary_df.to_csv('3) data quality evaluation/qualitative distribution assesment/bioresponse/stats/summary_stats_' + generator, index=False)
...
def get_scatters(real_data, synthetic_data, datatype, generator, feature_position):
    ...
    fig.savefig('3) data quality evaluation/qualitative distribution assesment/bioresponse/scatter plots/scatter_plots_'+names[feature_position] + '_' + generator + '.png')
    plt.close(fig)
...
compare_data(real_fraud_i, syn_fraud_i, 'no response', 'WcGAN')
