...
real_data = real_data.astype('float64')
...
def compare_data(real_data, synthetic_data, datatype, generator):
    ...
    #            transform=axes[i].transAxes)
    #
    # f.savefig('3) data quality evaluation/qualitative distribution assesment/customer churn/histograms/histograms_' + generator + '.png', bbox_inches='tight')
    ...
    correlation_matrix(real_corr - synthetic_corr, 'Correlation Difference for ' + generator+'\n Absolute mean correlation difference: '+ str(abs_mean_corr),
                       '3) data quality evaluation/qualitative distribution assesment/satisfaction/correlation plots/correlation_' + generator)

    summary_df = pd.DataFrame(data={'method': cols,
                                    'feature': cols,
                                    # 'pearsonr': pearsonrs,
                                    # 'intersection': intersections,
                                    'abs_mean_corr': abs_mean_corr})

    summary_df.to_csv('3) data quality evaluation/qualitative distribution assesment/satisfaction/stats/summary_stats_' + generator, index=False)
...
def get_scatters(real_data, synthetic_data, datatype, generator, feature_position):
    ...
    fig.savefig('3) data quality evaluation/qualitative distribution assesment/satisfaction/scatter plots/scatter_plots_'+names[feature_position] + '_' + generator + '.png')
    plt.close(fig)
...
compare_data(real_fraud_i, syn_fraud_i, 'not happy', data_name)
