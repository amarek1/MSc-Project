...
file_name = 'C:/Users/amarek/PycharmProjects/data_lab/datasets/TGAN_4_700.pkl'
syn_fraud = pd.read_pickle(file_name)

# # load the synthetic data
# file_name = 'C:/Users/amarek/Desktop/R/synthpop/syntpop_data_cart.csv'
...
ori_fraud = ori_data.loc[ori_data.loc[:, 'class'] == 1, :]

# syn_fraud = A.df
# syn_fraud['class'] = ori_fraud['class'].values
n_features = len(ori_data.columns)
...
def compare_data(original_data, synthetic_data, datatype, generator):
    ...
    f.savefig(datatype + '_histograms_' + generator + '.png', bbox_inches='tight')
    ...
    correlation_matrix(original_corr - synthetic_corr, datatype + 'Correlation Difference ' + generator,
                       datatype + '_correlation_' + generator)
    ...
    summary_df.to_csv(datatype + 'summary_stats_' + generator, index=False)
...
def get_scatters(original_data, synthetic_data, datatype, generator, feature_position):
    ...
    fig.savefig(datatype + '_scatter_plots_'+names[feature_position] + '_' + generator + '.png')
    plt.close(fig)
# # check for missing values
# print(ori_data.isnull().sum())
# print(syn_data.isnull().sum())


print(ori_fraud,syn_fraud)
# compare_data(ori_data, syn_data, 'f+nf', 'synthpop_cart')
ori_fraud_i = ori_fraud + 0.000001  # correct singular matrix error
syn_fraud_i = syn_fraud + 0.000001
compare_data(ori_fraud_i, syn_fraud_i, 'fraud', 'TGAN_4_2800')



ori_data = rescale_0_1(ori_data)
# syn_data = rescale_0_1(syn_data)
ori_fraud = rescale_0_1(ori_fraud)
...
for i in range(0, 31):
    get_scatters(ori_fraud, syn_fraud, 'fraud', 'TGAN_4_2800', i)
