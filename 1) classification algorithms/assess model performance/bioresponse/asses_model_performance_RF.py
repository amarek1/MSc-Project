...
path = '1) classification algorithms/random forest/bioresponse/model_forest_unbalanced_bio_short.pkl'
with open(path, 'rb') as file:
    unbalanced_model = pickle.load(file)

# unpack balanced model
path = '1) classification algorithms/random forest/bioresponse/model_forest_balanced_bio_short.pkl'
with open(path, 'rb') as file:
...
plt.savefig('1) classification algorithms/assess model performance/bioresponse/figures/PRcurve_rf_unbalanced_bio_short.png')
plt.close()
get_model_performance(balanced_model, 'balanced', X_test_balanced, y_test_balanced,'RF','bioresponse dataset')
plt.savefig('1) classification algorithms/assess model performance/bioresponse/figures/PRcurve_rf_balanced_bio_short.png')
plt.close()

cm_analysis(y_test_balanced, balanced_predictions, filename='1) classification algorithms/assess model performance/bioresponse/figures/cm_rf_balanced_bio_short.png',labels=[0, 1],
            ymap=['response','no response'],title='RF performance on balanced data\nbioresponse dataset')

cm_analysis(y_test_unbalanced,unbalanced_predictions,filename='1) classification algorithms/assess model performance/bioresponse/figures/cm_rf_unbalanced_bio_short.png',labels=[0, 1],
            ymap=['response','no response'],title='RF performance on unbalanced data\nbioresponse dataset')
