...
def plot_performance3(x_axis_steps=[0, 100, 200, 300, 381], report_dict=dict(), fraud_par='fraud', normal_par='normal',
    ...
    plt.grid()
    plt.legend()
    plt.savefig('4) performance improvement/credit card fraud/synthetic only/figures/plots/'+parameter+'_'+keys[len(keys)-1]+'.png')
    plt.close()

###################################### run the functions ###################################

synthpop_report = get_performance_report(dataset='credit card fraud', model_nr='rf', model_type='synthpop_so',
...
plot_performance3(x_axis_steps=[0, 100, 200, 300, 381], report_dict=synthpop_report, fraud_par='fraud', normal_par='normal',
                  parameter='precision', model='synthpop_so')
