...
synthetic_fraud = pd.concat([train_data.loc[train_data['class']==1]]*14, ignore_index=True)
...
nr_normal_training = [5381, 5381, 5381, 5381, 5381, 5381, 5381]  # number of real normal datapoints
nr_fraud_training = [381, 381, 381, 381, 381, 381,381]  # number of real fraud datapoints
...
for i in range(0, len(nr_normal_training)):
    get_forest_model(real_data=real_data, synthetic_data=synthetic_fraud, folder='control/models', model_name='rf',
                     model_type='control_fraud', nr_normal_training=nr_normal_training[i], nr_fraud_training=nr_fraud_training[i],
