...
def get_performance_report(dataset='satisfaction', model_nr='rf', model_type='real_only', nr_normal_training=[213224],
...
def plot_performance3(x_axis_steps=[0, 500, 1000, 2000, 3000, 4000, 5000], report_dict=dict(), fraud_par='fraud', normal_par='normal',
    ...
    plt.title('The effect of adding real fraud data on '+parameter)
    plt.grid()
    plt.legend()
    plt.savefig('4) performance improvement/satisfaction/real only/figures/plots/'+parameter+'_'+keys[len(keys)-1]+'.png')
    plt.close()

###################################### run the functions ###################################

control_report = get_performance_report(dataset='satisfaction', model_nr='rf', model_type='real_only',
                                        nr_normal_training=[8000, 8000, 8000, 8000],
                                        nr_fraud_training = [0, 1000, 2000, 3008],
                                        nr_synthetic_fraud_training = [0, 0, 0, 0])

plot_performance3(x_axis_steps=[0, 1000, 2000, 3008], report_dict=control_report, fraud_par='class1', normal_par='class0',
                  parameter='f1-score', model='control_sat')
...
plot_performance3(x_axis_steps=[0, 1000, 2000, 3008], report_dict=control_report, fraud_par='class1', normal_par='class0',
                  parameter='precision', model='control_sat')
...
plot_performance3(x_axis_steps=[0, 1000, 2000, 3008], report_dict=control_report, fraud_par='class1', normal_par='class0',
                  parameter='recall', model='control_sat')
