...
def get_performance_report(dataset='credit card fraud', model_nr='rf', model_type='real_only', nr_normal_training=[213224],
    ...
    for i in range(0, len(nr_normal_training)):
        ...
        path = '4) performance improvement/' + dataset + '/real only/models/' + model_name
        ...
        cm_analysis(y_test, model_predictions, filename='4) performance improvement/' + dataset + '/real only/figures/confusion matrices/cm_' + model_name + '.png',
...
def plot_performance3(x_axis_steps=[0, 500, 1000, 2000, 3000, 4000, 5000], report_dict=dict(), fraud_par='class1', normal_par='class0',
    ...
    plt.title('The effect of adding real bioresponse data on '+parameter)
    plt.grid()
    plt.legend()
    plt.savefig('4) performance improvement/bioresponse/real only/figures/plots/'+parameter+'_'+keys[len(keys)-1]+'.png')
    plt.close()

###################################### run the functions ###################################

control_report = get_performance_report(dataset='bioresponse', model_nr='rf', model_type='real_only',
                                        nr_normal_training=[2034, 2034, 2034, 2034, 2034],
                                        nr_fraud_training = [0, 100, 200, 300, 350],
                                        nr_synthetic_fraud_training = [0, 0, 0, 0, 0])

plot_performance3(x_axis_steps=[0, 100, 200, 300, 350], report_dict=control_report, fraud_par='class1', normal_par='class0',
                  parameter='f1-score', model='real_only')
...
plot_performance3(x_axis_steps=[0, 100, 200, 300, 350], report_dict=control_report, fraud_par='class1', normal_par='class0',
                  parameter='recall', model='real_only')
...
plot_performance3(x_axis_steps=[0, 100, 200, 300, 350], report_dict=control_report, fraud_par='class1', normal_par='class0',
                  parameter='precision', model='real_only')
