...
def plot_performance3(x_axis_steps=[0, 100, 200, 300, 381], report_dict=dict(), fraud_par='fraud', normal_par='normal',
    ...
    plt.title('The effect of adding synthetic bioresponse data on '+parameter)
    plt.grid()
    plt.legend()
    plt.savefig('4) performance improvement/bioresponse/synthetic only/figures/plots/'+parameter+'_'+keys[len(keys)-1]+'.png')
    plt.close()

###################################### run the functions ###################################

synthpop_report = get_performance_report(dataset='bioresponse', model_nr='rf', model_type='synthpop_so',
                                        nr_normal_training=[2034,2034,2034,2034,2034],
                                        nr_fraud_training = [0, 0, 0, 0, 0],
                                        nr_synthetic_fraud_training = [0, 100, 200, 300, 350])

plot_performance3(x_axis_steps=[0, 100, 200, 300, 381], report_dict=synthpop_report, fraud_par='class1', normal_par='class0',
