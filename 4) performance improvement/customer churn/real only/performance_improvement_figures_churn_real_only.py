...
def get_performance_report(dataset='customer churn', model_nr='rf', model_type='real_only', nr_normal_training=[213224],
...
def plot_performance3(x_axis_steps=[0, 500, 1000, 1500, 2000, 2500], report_dict=dict(), fraud_par='fraud', normal_par='normal',
    ...
    plt.title('The effect of adding real churn data on '+parameter)
    plt.grid()
    plt.legend()
    plt.savefig('4) performance improvement/customer churn/real only/figures/plots/'+parameter+'_'+keys[len(keys)-1]+'.png')
    plt.close()

###################################### run the functions ###################################

control_report = get_performance_report(dataset='customer churn', model_nr='rf', model_type='real_only',
                                        nr_normal_training=[3847, 3847, 3847, 3847, 3847, 3847],
                                        nr_fraud_training=[0, 300, 600, 900, 1200, 1435],
                                        nr_synthetic_fraud_training=[0, 0, 0, 0, 0, 0])

plot_performance3(x_axis_steps=[0, 300, 600, 900, 1200, 1435] , report_dict=control_report, fraud_par='fraud', normal_par='normal',
